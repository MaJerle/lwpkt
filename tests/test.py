...
def main(args):
    ...
    for file in files:
        ...
        retval |= subprocess.run(['cmake', '--fresh', '-DCMAKE_C_COMPILER=gcc', '-DCMAKE_CXX_COMPILER=gcc', '-S../..', '-G', 'Ninja', '-DTEST_CMAKE_FILE_NAME={}'.format(file)]).returncode
        print('Compile', flush=True)
        retval |= subprocess.run(['cmake', '--build', '.']).returncode
        print('Run test', flush=True)
        retval |= subprocess.run(['ctest', '.', '--output-on-failure']).returncode
